...
# 2. プレーヤー表示関数
def render_audio_player(audio_bytes: bytes, mime: str='audio/mpeg', autoplay=False, loop=False, height=90):
    if audio_bytes is None:
        st.write("音声ファイルが読み込まれていません。")
        return
    
    # ここでキャッシュされた関数を使う
    b64 = get_audio_base64(audio_bytes)
        ...
        for idx, a in enumerate(audio_list):
            ...
            with cols[1]:
                ...
                if st.button("再生", key=f"play_{a.get('safe_name')}_{idx}"):
                    audio_bytes, audio_mime = load_audio_file(a.get('path'))
                    render_audio_player(audio_bytes, mime=audio_mime, autoplay=True, loop=False, height=100)
            with cols[2]:
...
# stage (階層的色選択)
    ...
    if st.session_state['current_trial_index'] >= len(st.session_state['trials_order']):
        ...
        audio_mime = None
        if idx is not None:
            audio_info = st.session_state['audio_files'][idx]
            audio_bytes, audio_mime = load_audio_file(audio_info['path'])
...
# color picker (自由色選択)
    ...
    if st.session_state['color_trial_index'] >= len(st.session_state['color_trials_order']):
        ...
        audio_mime = None
        if cidx is not None:
            audio_info = st.session_state['audio_files'][cidx]
            audio_bytes, audio_mime = load_audio_file(audio_info['path'])
...
# final_survey
    ...
    with st.form("final"):
        ...
        # --- 追加ここから ---
        q19 = st.text_input("Q12.色聴共感覚の傾向 : 色聴共感覚（音を聞いたときに色を感じる知覚現象）について、ご自身にそのような傾向があると感じるか、可能な範囲で詳しく教えてください。")
        # --- 追加ここまで ---

...
# check page
    ...
    with col2a:
        ...
        右側のカラーピッカー（「:arrow_down: テスト用の色を選択」と書かれた色の四角）をクリックして、色を自由に選んでみてください。
                 
        この際、下部分の数値の欄は使わずに上の2つの色空間のみで選択してください。
        
        色を選んだ後、ピッカーの外側（この画面の白い部分など）をクリックしてピッカーを閉じてください。
        
        ピッカーを閉じると、選んだ色がその下のプレビュー領域に反映されていることを確認してください。
    ...
    実験は2つの異なる色選択タスク（「段階的な色選択」と「カラーピッカーでの色選択」）で構成されており、**提示される順序はランダムです**。"
    """
                )
